...
public:
    ...
    inline void PutVlqInt(uint64_t v) {
        while(!bit_writer.PutVlqInt(v)) {
            double_buffer();
        }
    }

    inline void PutZigZagVlqInt(int64_t v) {
...
struct DeltaBitPackEncoderConverter<format::Type::INT32> {
    ...
    inline int64_t as_int64_t(const int32_t* values, size_t index) {
...
struct DeltaBitPackEncoderConverter<format::Type::INT64> {
    ...
    inline int64_t as_int64_t(const int64_t* values, size_t index) {
        return values[index];
    }
...
public:
    ...
    void put(const typename DeltaBitPackEncoderConverter<ParquetType>::input_type* values, size_t values_len) {
        if (!values_len) {
            return;
        }
