...
declare -A name_filetype_map=([numerical]=numerical [strings]=strings [nested]=nested [mixed]=mixed [strings_gzip]=strings [strings_snappy]=strings)
declare -A name_rows_map=([numerical]=1250000 [strings]=250000 [nested]=175000 [mixed]=1000000 [strings_snappy]=250000 [strings_gzip]=250000)
declare -A name_rowgroups_map=([numerical]=50 [strings]=375 [nested]=45 [mixed]=50 [strings_snappy]=1850 [strings_gzip]=4625)
declare -A name_pagesize_map=([numerical]=8192 [strings]=8192 [nested]=8192 [mixed]=8192 [strings_snappy]=8192 [strings_gzip]=8192)
declare -A name_compression_map=([numerical]=uncompressed [strings]=uncompressed [nested]=uncompressed [mixed]=uncompressed [strings_snappy]=snappy [strings_gzip]=gzip)

declare -A plainness_name_map=([true]=plain [false]=dict)

for name in numerical mixed nested strings strings_snappy strings_gzip
do
	...
	do
		...
		(perf script -i ${arrow_filename}.perf | ${FLAMEGRAPH_DIR}/stackcollapse-perf.pl | ${FLAMEGRAPH_DIR}/flamegraph.pl > svg/${arrow_filename}.svg) 1>/dev/null
		...
		time ( perf record --call-graph dwarf -o ${seastar_filename}.perf \
			...
			--rows ${name_rows_map[${name}]} --rowgroups ${name_rowgroups_map[${name}]} --page ${name_pagesize_map[${name}]} \
			--string 8 --flba 8 \
			1>/dev/null 2>stalls/writer-${seastar_filename}.stalls \
		) 2>time/writer-${seastar_filename}.time
		(perf script -i ${seastar_filename}.perf | ${FLAMEGRAPH_DIR}/stackcollapse-perf.pl | ${FLAMEGRAPH_DIR}/flamegraph.pl > svg/${seastar_filename}.svg)  1>/dev/null
