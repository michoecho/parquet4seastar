...
name_type_map[strings_snappy]=strings
name_type_map[strings_gzip]=strings
	...
	do
		...
		do
			...
			do
				...
				(perf script -i reader-arrow-${filename}.perf | ${FLAMEGRAPH_DIR}/stackcollapse-perf.pl | ${FLAMEGRAPH_DIR}/flamegraph.pl > svg/reader-arrow-${filename}.svg) 1>/dev/null
				...
				time ( perf record --call-graph dwarf -o reader-seastar-${filename}.perf \
					...
					--blocked-reactor-notify-ms 1 --blocked-reactor-reports-per-minute 1000 2>stalls/reader-seastar-${filename}.stalls \
					1>/dev/null \
				) 2>time/reader-seastar-${filename}.time
				(perf script -i reader-seastar-${filename}.perf | ${FLAMEGRAPH_DIR}/stackcollapse-perf.pl | ${FLAMEGRAPH_DIR}/flamegraph.pl > svg/reader-seastar-${filename}.svg)  1>/dev/null
